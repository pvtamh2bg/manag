...
class config
{
	...
	function GetNewsAdmin($key,$per_page,$page,$type)
	{
		$l="";
		$offset = ($page - 1) * $per_page;
		if($type==0)
			$l="LIMIT ".$per_page." OFFSET ".$offset;
		   $sql = "SELECT * from qq_news WHERE concat(Name,Content) like '%$key%' ".$l;
	...
	function AddNews($Name,$Img,$Content,$User)
	{
	    $Name1=mysqli_real_escape_string($this->_conn,$Name);
	   $arr1 = array();
		
			$sql1 = "SELECT * from qq_news where Name='$Name1'";
		    $r1 = mysqli_query($this->_conn,$sql1);		
		
			while($a1 = mysqli_fetch_array($r1,MYSQLI_ASSOC))
			{
				$arr1[] = $a1;
			}			
		...
		$date=date('Y-m-d');
		$sql="INSERT INTO qq_news (Name,Img,Content,UpdateBy,UpdateDate) VALUES ('$Name1','$Img','$Content1','$User','$date')";
	...
	function UpdateNews($Id,$Name,$Img,$Content,$User)
	{
		...
		date_default_timezone_set("Asia/Ho_Chi_Minh");
		$date=date('Y-m-d');
		$query = "UPDATE qq_news SET Name='$Name1',Img='$Img',Content='$Content1',UpdateBy='$User',UpdateDate='$date' WHERE Id='$Id'";
	...
	function DeleteNews($id){
			...
			$sql="DELETE FROM qq_news WHERE Id = '$id'";
			mysqli_query($this->_conn, $sql);	
	...
	{
		$sql = "select Id from qq_feedback";
		$result =mysqli_query($this->_conn, $sql);	
		$num =0;
		$num = mysqli_num_rows($result);		
		return $num;
	}
	function CountNovelAll()
	{
	...
	{
	$sql = "SELECT sum(Sum_Views) as Sum_Views from qq_story";
		$result = mysqli_query($this->_conn, $sql);
		$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
		$sum=$row['Sum_Views'];
		return $sum;
	...
	function GetInfoUserID($IdUser)
	{		
		$sql = "SELECT Level,Email from qq_users where Id='$IdUser'";
		$result = mysqli_query($this->_conn, $sql);
		$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
		$Level=$row['Level'];
		$Email=$row['Email'];
	...
	function GetInfoAuthor($id)
	{
		$sql = "select * from qq_authors where Id='$id'";
		$result =mysqli_query($this->_conn, $sql);			
		$a = mysqli_fetch_array($result,MYSQLI_ASSOC);
		$c=array();
		if($a!=null){
	...
	function GetCommentAdmin($key,$per_page,$page,$type)
	{
		...
		if($type==0)
			$l="LIMIT ".$per_page." OFFSET ".$offset;
		   $sql = "SELECT * from qq_comments WHERE concat(`Content`) like '%$kk%' ".$l;
	...
	function GetUserAdmin($key,$per_page,$page,$type)
	{
		...
		if($type==0)
			$l="LIMIT ".$per_page." OFFSET ".$offset;
		   $sql = "SELECT * from qq_users WHERE Email like '%$kk%' ".$l;
	...
	function GetAuthorAdmin($key,$per_page,$page,$type)
	{
		...
		if($type==0)
			$l="LIMIT ".$per_page." OFFSET ".$offset;
		   $sql = "SELECT * from qq_authors WHERE Name like '%$kk%' ".$l;
	...
	function UpdateAdminUser($Id,$Level)
	{
...
$sql="INSERT INTO qq_comments (Content,Avatar,Name,Title,Likes,DateComment,IdChap,IdStory,IdUser) VALUES ('$Content_main', '$Avartar','$Name','$Title','$Likes','$DateComment','$IdChap','$IdStory','$IdUser')";
	...
	function UpdateAuthor($id,$Name,$NameEncode)
	{		
$sql="UPDATE qq_authors SET Name='$Name',NameEncode='$NameEncode' WHERE Id='$id'";
			//echo $sql;
			mysqli_query($this->_conn, $sql);
	}
	...
	function UpdateAuthorForStory($id,$Genre)
	{		
			$sql="UPDATE qq_story SET Author='$Genre' WHERE Id='$id'";
			//echo $sql;
			mysqli_query($this->_conn, $sql);
	}
	...
	function DeleteAuthorById($id){
			$sql="DELETE FROM qq_authors WHERE Id = '$id'";
			mysqli_query($this->_conn, $sql);		       
	}
