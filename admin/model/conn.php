...
class config
{
	...
	function check_story_exist($name,$Female,$Male)
	{
		...
		//$Male=1;truyen chu
		$sql = "select * from qq_story where Name='$name1' and Female='$Female' and Male='$Male'";
	    $result=mysqli_query($this->_conn,$sql);
		$i=0;
     	while($a = mysqli_fetch_array($result,MYSQLI_ASSOC))
		{
	...
	function GetNameStoryOFChap($name,$Female,$Male)
	{
		$sql = "select * from qq_story where Name='$name' and Female='$Female' and Male='$Male'";
		$result =mysqli_query($this->_conn, $sql);	
	...
	{		
		$query = "UPDATE qq_chapter SET `Path`='$Content',Content_04='$site' WHERE IdStory='$IdStory' and Name='$Name'";
		$result = mysqli_query($this->_conn, $query);
	}
	function UpdateChaptersNoel($IdStory,$Name,$Content)
	{		
		$query = "UPDATE qq_chapter SET `Content`='$Content' WHERE IdStory='$IdStory' and Name='$Name'";
		$result = mysqli_query($this->_conn, $query);
	}
	...
	function UpdateDomain_site($name_old,$name_new)
	{
	
    		$sql="UPDATE qq_site SET Name='$name_new' WHERE Name='$name_old'";	
    		mysqli_query($this->_conn, $sql);
	...
	function GetStoryByLink($name,$Female,$Male)
	{
