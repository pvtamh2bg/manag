	...
	if($arr[15]==1){
		...
		$color='style="background-color:red;"';
		 ...
		 echo '<a href="'.$linkOption.$the_loai.vn_str_filter($arr[1])."-".$arr[0].'" title="'.$arr[1].'">';						
		  echo '<img class="story-cover lazy_cover" src="'.$linkOption.'page/'.$arr[4].'" alt="'.$arr[1].'" style="width:190px;height:247px;"/>';
		 echo '</a>';
...
// function convert_timer($str){
	...
	// $DateComment="1 Phút Trước";
				 ...
				 // return $DateComment;
// }
function newsList($arr,$linkOption){	  
	 $db=new config();
     $db->config();	
	if(count($arr)>0){
	echo '<ul class="list-stories grid-6">';
	 foreach($arr as $arr3) {
		 $dateChap=$db->convert_timer1($arr3["UpdateDate"]);
	
	   echo '<li>';
			echo '<div class="story-item">';
				 echo '<a href="'.$linkOption.'tin-tuc-chi-tiet/'.vn_str_filter($arr3["Name"]).'-'.$arr3["Id"].'.html" title="'.$arr3["Name"].'">';						
				echo '<img class="story-cover lazy_cover" src="'.$linkOption.'page/'.$arr3["Img"].'" alt="'.$arr3["Name"].'" style="width:190px;height:247px;"/>';
				echo '</a>';
	            	echo '<div class="top-notice">';
			        echo '<span class="time-ago" '.$color.'>'.$dateChap.'</span>';
	            	if(strpos($dateChap, "Phút")!== false)
			         echo '<span class="type-label new">New</span>';
	        	echo '</div>';
				echo '<h3 class="title-book">';
					echo '<a href="'.$linkOption.'tin-tuc-chi-tiet/'.vn_str_filter($arr3["Name"]).'-'.$arr3["Id"].'.html" title="'.$arr3["Name"].'">'.ConvertStr($arr3["Name"],4).'</a>';
				echo '</h3>';
				echo '<div class="episode-book">';
					echo '<a href="'.$linkOption.vn_str_filter($arr3["Name"]).'.html">By: '.substr($arr3["UpdateBy"],0, strpos($arr3["UpdateBy"], '@')).'</a>';
				echo '</div>';
				echo '<div class="more-info">';
					echo '<div class="title-more">'.$arr3["Name"].'</div>';
					echo '<p class="info">Cập nhật bởi: '.substr($arr3["UpdateBy"],0, strpos($arr3["UpdateBy"], '@')).'</p>';
					echo '<div class="excerpt">'.ConvertStr($arr3["Content"],4).'</div>';
...
function ConvertStr($str,$index){
	...
	if($index==0){
		...
		if (mb_strlen($str, 'UTF-8') >10){
			$a=mb_substr($str,0, 10)."...";
		}
	}else if($index==4){
		if (mb_strlen($str, 'UTF-8') >400){
			$a=mb_substr($str,0, 400)."...";
		}
	}
	else{
