<style type="text/css">
  .Updates-module_subInnerWrapper_1P-W9 {
    background-color: #000;
    padding: 0 10px 22px 0;
    background: -webkit-gradient(linear, left top, left bottom, from(rgba(220, 9, 20, .5)), to(rgba(220, 9, 20, 0)));
    background: linear-gradient(180deg, rgba(220, 9, 20, .5), rgba(220, 9, 20, 0));
    width: 350px;
  }

  .Updates-module_rankingTitle_Ky0uL {
    padding-bottom: 10px;
    padding-left: 15px
  }

  .Updates-module_rankingTitle_Ky0uL:hover {
    background: -webkit-gradient(linear, left top, right top, from(rgba(0, 0, 0, .2)), to(transparent));
    background: linear-gradient(90deg, rgba(0, 0, 0, .2), transparent)
  }

  .RankingTitle-module_icon_hKj5L {
    height: 25px;
    width: auto;
    margin-top: 2px
  }

  .RankingTitle-module_viewCount_1E3Re {
    margin-top: 10px;
    font-size: 11px
  }

  .RankingTitle-module_rankingNumber_a7Dbg {
    line-height: 30px;
    background: var(--color-black);
    text-align: center;
    border-radius: 50%;
    width: 30px;
    height: 30px;
    display: block;
    position: relative;
    left: 20px;
    margin: 0 0 -12px 0;
    z-index: 1
  }

  .RankingTitle-module_rankingTitle_2E0E- {
    display: -webkit-box;
    display: -ms-flexbox;
    display: flex;
    color: #fff
  }

  .RankingTitle-module_rankingTitle_2E0E-:hover,
  .RankingTitle-module_rankingTitle_2E0E-:hover .RankingTitle-module_titleImg_18ZMt {
    opacity: 1
  }

  .RankingTitle-module_rankingTitle_2E0E-:hover .RankingTitle-module_title_2rg5n {
    color: var(--color-yellow)
  }

  .RankingTitle-module_rankingTitle_2E0E-:hover .RankingTitle-module_author_2DBfa {
    color: var(--color-white)
  }

  .RankingTitle-module_titleImg_18ZMt {
    height: 105px;
    opacity: .8
  }

  .RankingTitle-module_titleDetail_35zr8 {
    display: -webkit-box;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-orient: vertical;
    -webkit-box-direction: normal;
    -ms-flex-direction: column;
    flex-direction: column;
    margin-left: 10px
  }

  .RankingTitle-module_title_2rg5n {
    margin-top: 25px;
    font-size: 16px;
    line-height: 1.3rem
  }

  .RankingTitle-module_author_2DBfa {
    margin-top: 2px;
    color: var(--color-small-impact);
    font-size: 10px;
    letter-spacing: .05rem;
    text-overflow: ellipsis;
    font-weight: 300;
    text-transform: uppercase;
    line-height: .9rem
  }

  .RankingTitle-module_viewCount_1E3Re {
    margin-top: 10px;
    font-size: 11px
  }
  .RankingTitle-module_icon_hKj5L {
    height: 25px;
    width: auto;
    margin-top: 2px
  }

  .RankingTitle-module_rankingNumber_a7Dbg {
    line-height: 30px;
    background: var(--color-black);
    text-align: center;
    border-radius: 50%;
    width: 30px;
    height: 30px;
    display: block;
    position: relative;
    left: 20px;
    margin: 0 0 -12px 0;
    z-index: 1;
    color: var(--color-white);
    font-weight: 700;
  }
.RankingTitle-module_viewCountIcon_Haavo {
    margin-right: 3px !important;
    width: 10px !important;
    height: auto !important;
  }

  .Updates-module_subHeader_Nc89q {
    display: -webkit-box;
    display: -ms-flexbox;
    display: flex;
    width: 100%;
    -webkit-box-align: end;
    -ms-flex-align: end;
    align-items: flex-end;
    margin-bottom: calc(var(--space)*2);
    -ms-flex-wrap: wrap;
    flex-wrap: wrap
  }

  .Updates-module_rankingHeaderTitle__rR8m {
    padding: 30px 0 20px 25px;
    margin-right: auto;
    background: -webkit-gradient(linear, left top, right top, from(rgba(220, 9, 20, .5)), to(rgba(220, 9, 20, 0)));
    background: linear-gradient(90deg, rgba(220, 9, 20, .5), rgba(220, 9, 20, 0));
    position: relative;
    z-index: 1
  }

  .Updates-module_rankingHeaderTitle__rR8m:before {
    content: "";
    background-image: url("data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='80' height='96'%3E%3Cpath d='M18 22C13.3 25.9 2.7 38.1 4 66c4.5 30.5 36 30 36 30s35.3-.4 36-36S47 2 47 2s13.4 34-9 37c-22.8 2.6-20-17-20-17zm38 18s12.4 24-4 38c-3.2 3.7-22.9 8.6-28-4-5.2-15.2 7-18.9 12-19 21.4-.6 20-15 20-15z' fill='%23fff'/%3E%3C/svg%3E");
    background-repeat: no-repeat;
    background-position: 50%;
    background-size: cover;
    height: 30px;
    width: 24px;
    position: relative;
    top: -6px;
    left: -10px;
    float: left
  }

  .Updates-module_sideAd_Kq_1O {
    margin: 0 0 20px 0;
    border-radius: 0 0 0 25px;
    background: #000;
    background: -webkit-gradient(linear, left top, left bottom, from(transparent), to(rgba(0, 0, 0, .5)));
    background: linear-gradient(180deg, transparent, rgba(0, 0, 0, .5))
  }

  .Updates-module_sideAd_Kq_1O,
  .Updates-module_topAd_2rtgG {
    text-align: center;
    padding: 25px 10px 35px 10px;
    filter: progid:DXImageTransform.Microsoft.gradient(startColorstr="#000000", endColorstr="#000000", GradientType=1)
  }

  .Updates-module_topAd_2rtgG {
    margin: 10px 0 0 0;
    border-radius: 25px 0 0 0;
    background: #000;
    background: -webkit-gradient(linear, left top, left bottom, from(rgba(0, 0, 0, .5)), to(transparent));
    background: linear-gradient(180deg, rgba(0, 0, 0, .5), transparent)
  }

  .Updates-module_sideAd_Kq_1O div,
  .Updates-module_topAd_2rtgG div {
    width: 100%;
    max-width: 336px;
    margin: 0 auto;
    display: -webkit-box;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-align: center;
    -ms-flex-align: center;
    align-items: center
  }

  .Updates-module_bottomAd_kH-sI {
    display: none;
    text-align: center;
    padding: 25px 10px 35px 10px;
    margin: 0 0 20px 0;
    border-radius: 0 0 25px 25px;
    background: #000;
    background: -webkit-gradient(linear, left top, left bottom, from(transparent), to(rgba(0, 0, 0, .5)));
    background: linear-gradient(180deg, transparent, rgba(0, 0, 0, .5));
    filter: progid:DXImageTransform.Microsoft.gradient(startColorstr="#000000", endColorstr="#000000", GradientType=1)
  }

  .Updates-module_bottomAd_kH-sI img {
    width: 100%;
    max-width: 1170px;
    opacity: .4
  }

  .Updates-module_mpcBanner_H76yA {
    width: 100%;
    height: auto;
    border-radius: 10px 0 0 10px;
    -o-object-fit: cover;
    object-fit: cover
  }

  .Updates-module_mpcBanner_H76yA:hover {
    opacity: .85
  }

  .styles-module_title_3gUV9 {
    text-align: left;
    color: var(--color-white);
    font-size: var(--font-size-xxl);
    font-weight: 700;
    padding: 30px 50px 30px 25px;
    margin-right: auto;
    position: relative;
    z-index: 0
  }

  .Updates-module_subShowAll_IFDwn {
    display: block;
    color: var(--color-black) !important;
    -ms-flex-item-align: end;
    align-self: flex-end;
    padding: 5px 15px 5px 15px;
    background-color: var(--color-yellow);
    position: relative;
    right: -10px;
    top: -25px;
    border-radius: 7px 0 0 7px;
    text-decoration: none;
    -webkit-transition: all .3s ease;
    transition: all .3s ease;
  }

  .Updates-module_subShowAll_IFDwn:after {
    content: "";
    display: inline-block;
    width: 7px;
    height: 7px;
    color: var(--color-black);
    border-bottom: 2px solid var(--color-black);
    border-left: 2px solid var(--color-black);
    -webkit-transform: rotate(-135deg);
    transform: rotate(-135deg);
    margin-left: 5px;
    margin-bottom: 1px
  }

  .Updates-module_subShowAll_IFDwn:hover {
    text-decoration: none;
    padding: 5px 20px 5px 20px;
    border-radius: 15px 0 0 15px;
    -webkit-transition: all .3s ease;
    transition: all .3s ease
    color: var(--color-black) !important;
  }
  .Updates-module_subShowAllBottom_1qN-f {
  display:block;
  color:var(--color-red);
  text-decoration:none;
  padding:15px;
  border:1px solid var(--color-red);
  margin:20px 10%;
  text-align:center;
  opacity:.6;
  -webkit-transition:all .3s ease;
  transition:all .3s ease;
  letter-spacing:.05rem;
  text-transform:uppercase;
  border-radius:10px
}
.Updates-module_subShowAllBottom_1qN-f:after {
  content:"";
  display:inline-block;
  width:10px;
  height:10px;
  color:var(--color-black);
  border-bottom:3px solid var(--color-red);
  border-left:3px solid var(--color-red);
  -webkit-transform:rotate(-135deg);
  transform:rotate(-135deg);
  position:relative;
  left:7px;
  -webkit-transition:all .3s ease;
  transition:all .3s ease
}
.Updates-module_subShowAllBottom_1qN-f:hover {
  color:var(--color-yellow) !important;
  border:1px solid var(--color-yellow);
  opacity:1;
  -webkit-transition:all .3s ease;
  transition:all .3s ease;
  margin:20px 8%;
  font-weight: 500;
}
.Updates-module_subShowAllBottom_1qN-f:hover:after {
  left:17px;
  border-bottom:3px solid var(--color-yellow);
  border-left:3px solid var(--color-yellow);
  -webkit-transition:all .3s ease;
  transition:all .3s ease
}
</style>
<div class="Updates-module_subInnerWrapper_1P-W9">
  <div class="Updates-module_subHeader_Nc89q">
    <h1 class="styles-module_title_3gUV9 Updates-module_rankingHeaderTitle__rR8m">Hottest</h1>
    <a href="https://mangaplus.shueisha.tv" class="Updates-module_subShowAll_IFDwn">View All</a>
  </div>
  <div class="Updates-module_rankingTitle_Ky0uL">
    <a href="http://mangaplus.shueisha.tv/truyen-tranh/boruto-two-blue-vortex-45-en" class="">
      <p class="RankingTitle-module_rankingNumber_a7Dbg"><img
          alt="Read the Manga Boruto: Two Blue Vortex by Masashi Kishimoto / Mikio Ikemoto for free!"
          src="data:image/png;base64,"
          class="RankingTitle-module_icon_hKj5L"></p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga Boruto: Two Blue Vortex by Masashi Kishimoto / Mikio Ikemoto for free!"
          class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/boruto-two-blue-vortex-133177271965a354b7d02ec.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/boruto-two-blue-vortex-133177271965a354b7d02ec.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> Boruto: Two Blue Vortex</p>
          <p class="RankingTitle-module_author_2DBfa"> Masashi Kishimoto / Mikio Ikemoto</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo"/>342348</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL">
    <a href="http://mangaplus.shueisha.tv/truyen-tranh/one-piece-1-chap-1103-en.html" class="">
      <p class="RankingTitle-module_rankingNumber_a7Dbg"><img alt="Read the Manga One Piece by Eiichiro Oda for free!"
          src="data:image/png;base64,"
          class="RankingTitle-module_icon_hKj5L"></p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img alt="Read the Manga One Piece by Eiichiro Oda for free!"
          class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/one-piece-5246603956598ebba2a1b1.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/one-piece-5246603956598ebba2a1b1.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> One Piece</p>
          <p class="RankingTitle-module_author_2DBfa"> Eiichiro Oda</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">306427</p>
        </div>
      </div>
    </a>
  </div>
  <div class="Updates-module_rankingTitle_Ky0uL">
    <a href="http://mangaplus.shueisha.tv/truyen-tranh/jujutsu-kaisen-4-chap-247-en.html" class="">
      <p class="RankingTitle-module_rankingNumber_a7Dbg"><img  alt="Read the Manga Jujutsu Kaisen by Gege Akutami for free!"
          src="data:image/png;base64,"
          class="RankingTitle-module_icon_hKj5L"></p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga Jujutsu Kaisen by Gege Akutami for free!" class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/jujutsu-kaisen-747048761659927fa1fa45.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/jujutsu-kaisen-747048761659927fa1fa45.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> Jujutsu Kaisen</p>
          <p class="RankingTitle-module_author_2DBfa"> Gege Akutami</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">299972</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL">
    <a href="http://mangaplus.shueisha.tv/truyen-tranh/chainsaw-man-37-chap-152-en.html" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">4</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga Chainsaw Man by Tatsuki Fujimoto for free!" class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/chainsaw-man-318082924659d6d3955e85.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/chainsaw-man-318082924659d6d3955e85.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> Chainsaw Man</p>
          <p class="RankingTitle-module_author_2DBfa"> Tatsuki Fujimoto</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">292619</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="http://mangaplus.shueisha.tv/truyen-tranh/one-piece-1-chap-1103-en.html" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">5</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga Dragon Ball Super by Akira Toriyama / Toyotarou for free!"
          class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/dragon-ball-super-22881329065a5e1dab379b.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/dragon-ball-super-22881329065a5e1dab379b.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> Dragon Ball Super</p>
          <p class="RankingTitle-module_author_2DBfa"> Akira Toriyama / Toyotarou</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">198280</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="http://mangaplus.shueisha.tv/truyen-tranh/my-hero-academia-6-en" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">6</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga My Hero Academia by Kohei Horikoshi for free!" class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/my-hero-academia-267699271659929ce4f3e2.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/my-hero-academia-267699271659929ce4f3e2.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> My Hero Academia</p>
          <p class="RankingTitle-module_author_2DBfa"> Kohei Horikoshi</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">185725</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="http://mangaplus.shueisha.tv/truyen-tranh/spy-x-family-36-chap-93-en.html" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">7</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga SPY x FAMILY by Tatsuya Endo  for free!" class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/spy-x-family-957593451659c2127a7e47.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/spy-x-family-957593451659c2127a7e47.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> SPY x FAMILY</p>
          <p class="RankingTitle-module_author_2DBfa"> Tatsuya Endo </p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">159395</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="/titles/100003" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">8</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga Black Clover by Yuki Tabata for free!" class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/black-clover-10415169965a5213a2e5b8.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/black-clover-10415169965a5213a2e5b8.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> Black Clover</p>
          <p class="RankingTitle-module_author_2DBfa"> Yuki Tabata</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">143931</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="/titles/100274" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">9</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga Kagurabachi by Takeru Hokazono for free!" class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/kagurabachi-109633426665992f694eb50.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/kagurabachi-109633426665992f694eb50.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> Kagurabachi</p>
          <p class="RankingTitle-module_author_2DBfa"> Takeru Hokazono</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">136075</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="/titles/100191" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">10</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga 【OSHI NO KO】 by Aka Akasaka x Mengo Yokoyari for free!"
          class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/kagurabachi-109633426665992f694eb50.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/kagurabachi-109633426665992f694eb50.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> 【OSHI NO KO】</p>
          <p class="RankingTitle-module_author_2DBfa"> Aka Akasaka x Mengo Yokoyari</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">135865</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="/titles/100171" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">11</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img alt="Read the Manga Dandadan by Yukinobu Tatsu for free!"
          class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/dandadan-24528951865a55b5e4d4ee.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/dandadan-24528951865a55b5e4d4ee.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> Dandadan</p>
          <p class="RankingTitle-module_author_2DBfa"> Yukinobu Tatsu</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">99014</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="/titles/100110" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">12</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga KAIJU NO.8 (Monster #8) by Naoya Matsumoto for free!"
          class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/kaiju-no-8-monster--603380211659c24835b5ae.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/kaiju-no-8-monster--603380211659c24835b5ae.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> KAIJU NO.8 (Monster #8)</p>
          <p class="RankingTitle-module_author_2DBfa"> Naoya Matsumoto</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">98454</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="/titles/100290" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">13</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga Kaiju No. 8: B-Side by Naoya Matsumoto / Keiji Ando / Kentaro Hidano for free!"
          class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/kaiju-no-8-b-side-136331646265993182751d5.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/kaiju-no-8-b-side-136331646265993182751d5.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> Kaiju No. 8: B-Side</p>
          <p class="RankingTitle-module_author_2DBfa"> Naoya Matsumoto / Keiji Ando / Kentaro Hidano</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">85903</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="/titles/100127" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">14</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga SAKAMOTO DAYS by Yuto Suzuki for free!" class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/sakamoto-days-891301117659926c59e675.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/sakamoto-days-891301117659926c59e675.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> SAKAMOTO DAYS</p>
          <p class="RankingTitle-module_author_2DBfa"> Yuto Suzuki</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">81493</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="/titles/100289" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">15</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga The God Before Me by Shou Kunoda for free!" class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/the-god-before-me-558006062659c14341046e.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/the-god-before-me-558006062659c14341046e.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> The God Before Me</p>
          <p class="RankingTitle-module_author_2DBfa"> Shou Kunoda</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">64644</p>
        </div>
      </div>
    </a></div>
  <div class="Updates-module_rankingTitle_Ky0uL"><a href="/titles/100198" class="">
      <p class="RankingTitle-module_icon_hKj5L RankingTitle-module_rankingNumber_a7Dbg">16</p>
      <div class="RankingTitle-module_rankingTitle_2E0E-"><img
          alt="Read the Manga Make the Exorcist Fall in Love by Aruma Arima/Masuku Fukayama for free!"
          class="RankingTitle-module_titleImg_18ZMt"
          data-src="https://mangaplus.shueisha.tv/page/upload/story/190x247/make-the-exorcist-fall-in-love-1963071237659d7072769b1.jpg"
          src="https://mangaplus.shueisha.tv/page/upload/story/190x247/make-the-exorcist-fall-in-love-1963071237659d7072769b1.jpg"
          lazy="loaded">
        <div class="RankingTitle-module_titleDetail_35zr8">
          <p class="RankingTitle-module_title_2rg5n"> Make the Exorcist Fall in Love</p>
          <p class="RankingTitle-module_author_2DBfa"> Aruma Arima/Masuku Fukayama</p>
          <p class="RankingTitle-module_viewCount_1E3Re"><img alt="hot manga at shueisha tv" src="<?php echo $linkOption1; ?>frontend/images/hot.svg"
              class="RankingTitle-module_viewCountIcon_Haavo">49164</p>
        </div>
      </div>
    </a>
  </div>
  <a href="https://mangaplus.shueisha.tv" class="Updates-module_subShowAllBottom_1qN-f">View All</a>
</div>