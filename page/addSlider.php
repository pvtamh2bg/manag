 ...
 <div class="column columns">
                ...
                <div class="user-main column">
                    <div class="level title">
                        <p class="level-left has-text-weight-bold">Thêm slider</p>
                    </div>
...
$(document).ready(function(){
	...
	$("#addSlider").click(function(){
		var path=$('#addSlider').attr("src-image");
		var idStory = $('#selUser').val();
	  $.ajax({     
	   url:linkOption12+'ajax/slider/add.php',
	   type:'POST',
	   cache:false,
	   data:{'idStory':idStory,'path':path},
	   success:function(kq){
		   var o = JSON.parse(kq);
			...
			//console.log(k.name);
				if(k.path==""){
					...
					if($('#inputDelImage').val() == ''){
						...
						$("#addSlider").attr("src-image",k.path);
